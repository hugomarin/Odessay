...
const states: DocumentState[] = ["cloud-only", "local-only", "synced", "pending"]
    ...
    <main className="min-h-screen bg-bg px-10 py-10 text-ink">
      ...
      >
        <div className="mb-5 flex items-center justify-between border-b-[0.5px] border-border pb-4">
          <h1 className="font-lora text-[22px] font-medium text-ink">Document state badges</h1>
        ...
        <div className="grid gap-3">
          {states.map((state) => (
            ...
            >
              <span className="text-[12px] font-medium uppercase tracking-[0.07em] text-ink-4">{state}</span>
              <WritingStatusBadge status="draft" variant="compact" className="justify-self-start" />
              <DocumentStateBadge state={state} variant="compact" className="justify-self-start" />
            </div>
          ))}
        </div>
      </section>
