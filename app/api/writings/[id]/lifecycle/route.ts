...
const inputSchema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("restore"), updated_at: z.string().datetime() }),
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  const supabase = await createClient()
  const archived = await supabase.from("writings").select("id").eq("id", id).eq("author_id", userId).not("deleted_at", "is", null).maybeSingle()
  ...
  if (parsed.data.action === "restore") {
    const { data, error } = await supabase.from("writings").update({ deleted_at: null, updated_at: parsed.data.updated_at, sync_status: "synced" }).eq("id", id).eq("author_id", userId).select().single()
    return error ? failure(500, "DB_ERROR", error.message) : NextResponse.json({ data, error: null })
  }
