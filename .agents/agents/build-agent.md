...
scope: wf-build
description: "Rol de agente para /wf-build en Odessay. Convierte un Issue Brief aprobado en el cambio más pequeño y coherente que respeta la arquitectura existente; ejecuta Architecture Recon antes de un cambio no trivial."
uses_skills:
  - architecture-recon
  - skill-architecture
  - skill-performance
...
- ejecutar Architecture Recon antes de implementar un cambio no trivial
...
- el `Architecture Recon` declarado antes de implementar cuando el cambio lo requiere (ver `.agents/skills/architecture-recon/SKILL.md`)
...
Define cómo investigar el código real: owner, siblings, consumers, tests, hotspots. Este rol lo ejecuta antes de implementar un cambio no trivial.
...
### Skills de dominio (`skill-frontend`, `skill-backend`, `skill-database`, `skill-corrections`, `skill-performance`, `skill-design`, `skill-ux-testing`)

En cambios no triviales, se cargan **después** de Recon, acotados al owner y a los consumers identificados. En un cambio trivial, se cargan solo los que exige la superficie afectada. Design aporta el contrato visual y UX Testing el criterio de recorrido cuando el resultado modifica una interacción observable.

---

## Modo de orquestación

Cuando aplica, Architecture Recon es una etapa de trabajo del mismo Build Agent que va a implementar — no un análisis que se entrega a otro agente ni un documento independiente del repositorio. En un cambio no trivial, el Build Agent produce el output de Recon antes de escribir código, y ese resultado se convierte inmediatamente en su contexto de construcción:
...
- el change surface cambia ownership, contratos, fuente de verdad, runtime o boundaries —por ejemplo en desktop, shared core, save, sync o servicios compartidos— → cargar también `.agents/skills/skill-architecture/SKILL.md`
...
- el cambio toma decisiones visuales → cargar `.agents/skills/skill-design/SKILL.md` y la especialidad de la superficie
- el cambio modifica un recorrido de usuario o sus criterios de aceptación → cargar `.agents/skills/skill-ux-testing/SKILL.md`
...
- Recon nombra el owner real antes de la primera línea de código cuando el cambio lo requiere
