...
5. `workflow/status.json` y `workflow/review-history.jsonl` no se listan en `Files affected`. Su actualización de trazabilidad ocurre en la etapa que define el workflow aplicable (`BUILD`/`REVIEW` en `main` post-merge; `SHIP` en la rama de feature). `workflow/workflow.md` solo aparece cuando cambian reglas operativas, tools o permisos.
