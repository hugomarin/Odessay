...
import {
  getEditorFootnotes,
  getMarkdownWithFootnoteDefinitions,
} from "@/lib/editor/footnote-node";
import {
...
const splitCanonicalDocumentSource = (
  ...
  return {
    frontmatter: normalizedSource.slice(
      FRONTMATTER_DELIMITER.length + 1,
      closingIndex,
    ),
...
const normalizeDocumentMarkdown = (source: string) => {
  ...
  if (frontmatter) {
    return restoreForeignFrontmatter(
      frontmatter,
      normalizeMarkdownForRoundTrip(markdown),
    );
  }
...
export const serializeCanonicalFrontmatter = (
...
export const parseMarkdownToSnapshot = (
  markdown: string,
): DocumentSerializationSnapshot => {
  const { frontmatter, markdown: bodyMarkdown } =
    splitCanonicalDocumentSource(markdown);
...
export const parseDocumentFileToSnapshot = (
  source: string,
): CanonicalDocumentFileSnapshot => {
  return {
    metadata: null,
