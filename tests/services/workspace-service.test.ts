...
vi.mock("@tauri-apps/api/path", () => ({ appConfigDir: vi.fn(async () => "/tmp/cfg") }))
vi.mock("@tauri-apps/plugin-dialog", () => ({ open: vi.fn(async () => null) }))
vi.mock("@/lib/editor/extensions", () => ({ EMPTY_EDITOR_JSON: { type: "doc" } }))
...
describe("DesktopWorkspaceService assignments", () => {
  ...
  it("assigns and then moves a writing, replacing the prior workspace", async () => {
    ...
    expect(settings.store.workspaceAssignments).toEqual({ "writing-1": "letters" })
  })

  it("rejects assignment to an unknown workspace", async () => {
    await expect(service.assignToWorkspace("writing-1", "ghost")).rejects.toThrow(/Unknown workspace/)
  })
