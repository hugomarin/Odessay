...
import { OPTIONS, POST } from "@/app/api/ai/workspace-document-relations/route"

describe("/api/ai/workspace-document-relations", () => {
  it("delegates to the shared semantic round with a forced relations operation", async () => {
    const request = new Request("https://app.odessay.test/api/ai/workspace-document-relations", {
      method: "POST",
      headers: { authorization: "Bearer token", "content-type": "application/json" },
      body: JSON.stringify({ operation: "merge", input: [{ type: "message", role: "user", content: "Review claims." }], tools: [] }),
    })
    ...
    expect(JSON.parse(forwardedBody.value)).toMatchObject({ operation: "relations" })
  })

  it("preserves the shared CORS preflight path", async () => {
    const request = new Request("https://app.odessay.test/api/ai/workspace-document-relations", { method: "OPTIONS" })
