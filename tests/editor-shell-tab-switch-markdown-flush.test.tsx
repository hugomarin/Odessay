...
const topbarState = vi.hoisted(() => ({
  ...
  onSelectTab: null as ((tabId: string) => void | Promise<void>) | null,
}))

const markdownContentState = vi.hoisted(() => ({
  onMarkdownChange: null as ((nextMarkdown: string) => void) | null,
}))
...
vi.mock("@/components/editor/editor-content", () => ({
  WritingEditorContent: (props: { onMarkdownChange?: (nextMarkdown: string) => void }) => {
    markdownContentState.onMarkdownChange = props.onMarkdownChange ?? null
    return null
  },
}))
...
beforeEach(async () => {
  ...
  markdownContentState.onMarkdownChange = null
...
describe("ODE-478 case 2 — tab switch must not attribute a pending edit to the wrong document", () => {
  ...
  it("does not hydrate another tab until the outgoing annotation snapshot is durable", async () => {
    persistedSession.value = {
      id: "workspace",
      active_tab_id: "doc-a",
      tabs: [
    ...
    const annotationJson = {
      ...
      content: [{
        ...
        content: [{
          type: "text",
          text: "fragmento anotado",
          marks: [{
            type: "highlight",
            attrs: {
              annotationId: "ann-1",
              annotationType: "personal",
              annotationComment: "nota persistente",
            },
          }],
        }],
      }],
    ...
    await act(async () => {
      editorState.text = "fragmento anotado"
      editorState.json = annotationJson
      editorState.isEmpty = false
      editorState.capturedOnUpdate?.({ editor: editorStub })
      switchPromise = topbarState.onSelectTab?.("doc-b") as Promise<void> | undefined
    })

    await vi.waitFor(() => expect(mocks.saveWriting).toHaveBeenCalledTimes(1), { timeout: 4500 })
    expect(mocks.openWriting).not.toHaveBeenCalledWith("doc-b")
    expect(mocks.saveWriting.mock.calls[0]?.[0].writing.content.richText).toEqual(annotationJson)
...
describe("ODE-531 regression — pending Source-mode annotation must flush to the outgoing tab on tab switch", () => {
  it("persists a markdown annotation typed <800ms before switching tabs under the outgoing writing id", async () => {
    persistedSession.value = {
      id: "workspace",
      active_tab_id: "doc-a",
      tabs: [
        { id: "doc-a", writing_id: "doc-a", slug: null, title: "Doc A", save_state: "saved", has_pending_sync: false, last_touched_at: 2, view_state: { mode: "markdown", markdownSelectionStart: 0, markdownSelectionEnd: 0, scrollTop: 0, scrollLeft: 0, windowScrollX: 0, windowScrollY: 0, shellScrollTop: 0, shellScrollLeft: 0 } },
        { id: "doc-b", writing_id: "doc-b", slug: null, title: "Doc B", save_state: "saved", has_pending_sync: false, last_touched_at: 1, view_state: null },
      ],
      recent_writings: [],
      updated_at: 1,
    ...
    await act(async () => root?.render(<EditorShell />))
    await vi.waitFor(() => expect(markdownContentState.onMarkdownChange).not.toBeNull())
    await vi.waitFor(() => expect(mocks.openWriting).toHaveBeenCalledWith("doc-a"))

    // Simulate typing an annotation in Source mode: the 800ms markdown save
    // debounce is now pending. Switching to B in the SAME tick must flush it
    // against doc-a before the identity flips — exactly like ODE-478 case 2
    // does for the queued rich-mode update.
    const typedMarkdown = "Doc A body\n\n<Annotation id=\"ann-1\" type=\"personal\" comment=\"nota\">fragmento</Annotation>"
    await act(async () => {
      editorState.text = "Doc A body fragmento"
      markdownContentState.onMarkdownChange?.(typedMarkdown)
      topbarState.onSelectTab?.("doc-b")
    })
    ...
    // The annotation must never be attributed to doc-b either.
    const contaminated = mocks.saveWriting.mock.calls.find(
      (call) => call[0].writing.id === "doc-b" && call[0].writing.content.plainText === "Doc A body fragmento",
    )
