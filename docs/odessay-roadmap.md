...
Setup de Next.js 14+ con App Router. Tailwind CSS instalado. ShadCN instalado sin estilos default. Estructura de carpetas según CLAUDE.md. El proyecto hace build sin errores.
...
Migraciones iniciales para todas las tablas: profiles, writings, correspondences, collections, writing_collections, writing_shares, ai_observations, invitations. RLS en todas las tablas. Triggers base: on_auth_user_created, slug generation, body_text extraction, correspondence creation.
Dependencias: Configure Supabase projects.
...
Al terminar esta fase: un usuario puede registrarse, abrir el editor, escribir, ver el auto-save funcionando, y encontrar sus writings en /home.
...
**Implement auto-save to Supabase with debounce** `[backend, database]`
onUpdate de TipTap con debounce de 1-2 segundos. PATCH a API route que actualiza body_json, body_text y updated_at. Indicador visual mínimo (punto sutil, sin texto). Fire-and-forget con retry silencioso si falla. El usuario no debe notar latencia ni ansiedad de pérdida de datos.
Dependencias: Implement TipTap editor.
Referencia: skill-backend.md, odessay-modelo-datos.md (sección: writings).

**Build /home — personal writing desk** `[frontend, backend]`
Vista privada del autor. Lista de todos sus writings (drafts y finished, cualquier visibilidad). Acceso rápido a crear nuevo writing. Filtros por estado y visibilidad. Notificaciones sutiles de nuevas respuestas o cartas recibidas.
...
Dependencias: Build /home.

---

## Fase 2 — Compartir y leer

Al terminar esta fase: un usuario puede hacer un writing público o compartirlo con alguien específico, y ese alguien puede leerlo en un espacio dedicado.
