...
**Enmienda (ODE-400, 2026-07-21) — "en su sitio" ≠ inamovible; el eje es la intención del usuario.** "En su sitio" prohíbe que el sistema reubique archivos **sin intención del usuario** (el repliegue de rehome automático y de `writingsDir` como dueño). NO prohíbe un **movimiento explícito iniciado por el usuario** a través de Artifact Studio. Ese move es *consciente* y **debe preservar el binding**, porque el reconciler lo reconoce por UUID/inode/content_hash (match, no detach+create):

- **Save to disk / Save As desde el managed root** (primer guardado real de un documento nuevo, que hoy nace en Application Support sin que el usuario lo perciba como guardado): es un **MOVE físico (rename), no una copia**. Cero residuo en el managed root; **un solo `canonical_path`** en todo momento; mismo UUID.
- **Move to workspace respaldado por carpeta:** mueve el `.md` a la carpeta del BindingRoot destino, actualiza el ledger `.odessay/index.json` de **origen y destino** y el binding en SQLite en una transacción. La membresía queda **inferida por ruta** (`inferWorkspaceSlugFromPath`).

**Acotación de la asignación metadata-only (ODE-318 / `lib/workspace/assignment.ts`):** la regla "asignar/mover a un workspace nunca reubica el archivo en disco" aplica **solo a workspaces de pura presentación** (sin carpeta de respaldo). Para workspaces **respaldados por carpeta**, "Move to" ES un relocate físico; tras el move debe **limpiarse la asignación explícita (`workspaceAssignments`) stale** para no sostener dos verdades de membresía (inferida-por-ruta vs metadata). El flujo "Connect Root / Use an existing folder" no cambia: adopta una carpeta existente sin mover archivos.
