...
Lee `workflow/context/core/odessay-fundacional.md` para entender la identidad epistolar, `workflow/context/core/odessay-paginas.md` para la arquitectura de rutas, y `workflow/context/core/odessay-modelo-datos.md` para el schema de `profiles` y `writings`.
...
- Márgenes en la reading view → `workflow/context/features/odessay-margenes.md`
- Correspondencias y navegación del árbol → `workflow/context/features/odessay-correspondencias.md`
- Collections públicas (lógica de colección) → `workflow/context/features/odessay-collections.md`
- Mecanismo de responder (reply_to, parent_id) → `workflow/context/core/odessay-paginas.md` §Mecanismo universal de respuesta
- Configuración del perfil (`/settings`) → `workflow/context/core/odessay-paginas.md` §/settings
