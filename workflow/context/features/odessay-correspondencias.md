...
- `body_text` — texto plano **derivado** (de la copia de trabajo `body_json`, que a su vez deriva del `.md` canónico). No es fuente de verdad: los anclajes de márgenes se resuelven desde `<Annotation>` y su `id` estable inline; el reader legacy todavía acepta highlight + sigil durante la migración (ver `odessay-adr-identidad.md` D1/D3 y `odessay-margenes.md`)
